...
module.exports = sock = async (sock, m, msg, chatUpdate, store) => {
	...
	try {
		...
		if (!m.isGroup && !isCreator && !isPremium && !isSuperAdmin && db.data.settings[botNumber].onlygc) {
			if (command) {
		...
		if (!isCreator && !isSuperAdmin && db.data.settings[botNumber].onlypc && m.isGroup) {
			if (command) {
				let captionText = "⚠️ *「 WARNING 」* ⚠️\n";
		...
		if (!sock.public) {
			if (!isCreator && !isSuperAdmin && !m.key.fromMe) return;
		};
		...
		if (db.data.chats[m.chat].antibot) {
			if (m.isBaileys) {
				if (m.key.fromMe || isAdmins || isCreator || isSuperAdmin || !isBotAdmins || isWhitelist) return;
		...
		if (db.data.chats[m.chat].antispam) {
			if (m.isGroup && m.message && isFiltered(m.chat)) {
				...
				if (m.key.fromMe || isAdmins || isCreator || isSuperAdmin || !isBotAdmins) return;
		...
		if (db.data.settings[botNumber].safesearch && command && !m.key.fromMe && !isCreator && !isSuperAdmin) {
			try {
		...
		if (!isCreator && !isSuperAdmin && !m.key.fromMe && m.message) {
		...
		if (m.isGroup) {
			if (db.data.chats[m.chat].mute && !isCreator && !isSuperAdmin) {
				return;
			}
		}
		...
		if (isCmd) {
			if (isFiltered(m.chat) && !isPremium && !isCreator && !isSuperAdmin && !m.key.fromMe) return newReply(`Don't spam! please give pause for a few seconds.`);
			addFilter(m.chat);
		}
		...
		if (isCmd) {
			if (command) {
				...
				if (onlyRegister) {
					if (!(command === "daftar") && !isCreator && !isAdmins && !isSuperAdmin) { 
		...
		if (isGroupBlacklist && !isSuperAdmin) {
			if (!isCmd) return;
		...
		switch (command) {
			...
			case 'nsfw': {
				...
				if (!isAdmins && !isGroupOwner && !isCreator && !isSuperAdmin) return newReply(mess.admin);
				if (args[0] === 'on') {
			...
			case 'antitoxic': {
				...
				if (m.isGroup && !isAdmins && !isCreator && !isSuperAdmin && command !== 'autoaipc') return newReply("❗ Hanya admin yang bisa mengatur fitur ini!");
			...
			case 'cleardesk':{
				...
				if (!isAdmins && !isGroupOwner && !isCreator && !isSuperAdmin) return newReply('Perintah ini hanya dapat digunakan oleh admin.');
				try {
			...
			case 'tagall': {
				...
				if (!isAdmins && !isGroupOwner && !isCreator && !isSuperAdmin) return newReply(`Maaf, kak! Kamu harus jadi admin dulu buat pakai fitur ini.`)
			...
			case 'd':
			case 'del': {
				if (!isAdmins && !isCreator && !isPremium && !isSuperAdmin) return newReply('Gagal menghapus pesan, hanya owner dan user premium yang dapat menghapus pesan!');
			...
			case 'dellist': {
				...
				if (!isAdmins && !isCreator && !isSuperAdmin) return newReply(`Hanya admin atau creator yang bisa menghapus list, sorry ya! 😜`);
				try {
					const keys = Object.keys(db.data.chats[m.chat].liststore);
			...
			case 'addlist': {
				...
				if (!isAdmins && !isCreator && !isSuperAdmin) return newReply(`Hanya admin atau creator yang bisa menambah list. 😇`);
			...
			case 'update': {
				...
				if (!isAdmins && !isCreator && !isSuperAdmin) return newReply(`Cuma admin atau creator yang bisa update list. 😇`);
				let args1 = q.split("|")[0].toLowerCase();
				let args2 = q.split("|")[1];
				if (!q.includes("|")) return newReply(`Gunakan format yang benar!*Kirim perintah*: ${prefix + command} *key|product*\n*Kirim perintah*: ${prefix + command} hello|hi`);
